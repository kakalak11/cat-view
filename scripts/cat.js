...
function App() {
    ...
    const listItems = van.derive(() => {
        if (!savedImages.val.length) {
            ...
            return renderList(savedImages.val);
        }
    });
    ...
    van.derive(() => {
        if (imgData.val) {
            Array.from(listItems.val.children).forEach((listItem) => {
                ...
                if (imgData.val.id == listItem.getAttribute("id")) {
    ...
    function saveImage(evt) {
        if (!_localStorage || !imgData.val) return;

        let favCats = _localStorage.getItem(storageKey);
        if (favCats) {
            favCats = JSON.parse(favCats);
            if (favCats.indexOf(imgData.val.id) === -1) {
                favCats = favCats.concat(imgData.val.id);
            }
